...
/* The position of the MB Error ALOGIc Switch
 * */
#define MB_ERROR_ALOGIC_SWITCH_POS (16)
...
int NormalRunningTest(int argc, char** argv, MYDATATYPE *pTempAppData)
{
    ...
    /* Set/Clear the MB Error Checking ALOGIc
     * */
    if (argc > MB_ERROR_ALOGIC_SWITCH_POS && pAppData->nTestCase != 10){
        pAppData->MB_Error_ALOGIc_Switch = atoi(argv[MB_ERROR_ALOGIC_SWITCH_POS]);
    }
    else {
       /* If the parameter is not provided the macroblock error information will
        * be Disabled.
        * */
        pAppData->MB_Error_ALOGIc_Switch = 0;
    }
#ifdef __GET_BC_VOP__
    if( pAppData->nTestCase == 10 ){
        if(argc == 18)
    ...
    do
    {
        ...
        if (pAppData->nTestCase == TESTCASE_TYPE_MBERRORCHECK || pAppData->MB_Error_ALOGIc_Switch) {
            ...
            /* Enable/Disable MB Error ALOGIc on Below Layers
             * */
            pMBErrorType.bEnabled         = pAppData->MB_Error_ALOGIc_Switch;
        ...
        while ((eError == OMX_ErrorNone) && (pAppData->eState != OMX_StateIdle) && (pAppData->eState != OMX_StateInvalid)) {
            ...
            if (FD_ISSET(pAppData->OpBuf_Pipe[0], &rfds) && (!pAppData->bDeinit) ) {
                ...
                if(pBuf != NULL) {
                    ...
                    /* If the MB Error ALOGIc is enabled, print the MB Errors
                     * */
                    if (pAppData->MB_Error_ALOGIc_Switch){
            ...
            if (pAppData->bDeinit) {
                ...
                if (eError != OMX_ErrorNone) {
                 ...
                 /* If the MB Error Code ALOGIc is Enabled Print the Error Num.
                  * */
                 /***************** MBError Code ********************/
                 if (pAppData->nTestCase == TESTCASE_TYPE_MBERRORCHECK || pAppData->MB_Error_ALOGIc_Switch){
                    if (bMBErrorCount == 0){
