        ...
        else if ((OMX_U32)args [0] == EMMCodecOuputBuffer)
        {
            ...
            if (!WMADEC_IsValid(pComponentPrivate_CC,pBuffer,OMX_DirOutput))
            {
                /* If the buffer we get back from the DSP is not valid call FillBufferDone
                   on a valid buffer */
#ifdef __PERF_INSTRUMENTATION__
                int nInvalidFrameCnt=pComponentPrivate_CC->nInvalidFrameCount++;
                PERF_SendingFrame(pComponentPrivate_CC->pPERFcomp,
                                  pComponentPrivate_CC->pOutputBufferList->pBufHdr[nInvalidFrameCnt]->pBuffer,
                                  pComponentPrivate_CC->pOutputBufferList->pBufHdr[nInvalidFrameCnt]->nFilledLen,
                                  PERF_ModuleHLMM);
#endif
